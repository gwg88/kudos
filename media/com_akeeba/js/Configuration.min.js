...
akeeba.Configuration.onBrowser=function(a,b){akeeba.Configuration.onBrowserCallback=function(a){b.value=a;"object"==typeof akeeba.Configuration.fsBrowser.modalObject&&akeeba.Configuration.fsBrowser.modalObject.close&&akeeba.Configuration.fsBrowser.modalObject.close()};var d=akeeba.Configuration.URLs.browser+encodeURIComponent(a),e=document.getElementById(akeeba.Configuration.fsBrowser.params.dialogBodyId);e.innerHTML="";var c=document.createElement("iframe");c.setAttribute("src",d);c.setAttribute("width",
"100%");c.setAttribute("height",400);c.setAttribute("frameborder",0);c.setAttribute("allowtransparency","true");e.appendChild(c);akeeba.Configuration.fsBrowser.modalObject=akeeba.Modal.open({inherit:"#"+akeeba.Configuration.fsBrowser.params.dialogId,width:"80%"})};
akeeba.Configuration.FtpBrowser.callback=function(a){var b="/".replace(/([\[\]\(\)\.\?\/\*\{\}\+\$\^\:])/g,"$1");a="/"+(a+"").replace(RegExp("^["+b+"]+","g"),"");document.getElementById("var["+akeeba.Configuration.FtpBrowser.params.key+"]").value=a};
...
(akeeba.Configuration.SftpBrowser.params.directory="");akeeba.System.doAjax({host:akeeba.Configuration.SftpBrowser.params.host,port:akeeba.Configuration.SftpBrowser.params.port,username:akeeba.Configuration.SftpBrowser.params.username,password:akeeba.Configuration.SftpBrowser.params.password,directory:akeeba.Configuration.SftpBrowser.params.directory,privkey:akeeba.Configuration.SftpBrowser.params.privKey,pubkey:akeeba.Configuration.SftpBrowser.params.pubKey},function(a){var d=document.getElementById("sftpBrowserFolderList"),
e=document.getElementById("ak_scrumbs");d.style.display="none";e.style.display="none";if(!1!=a.error)document.getElementById("sftpBrowserError").innerHTML=a.error,document.getElementById("sftpBrowserErrorContainer").style.display="block";else{document.getElementById("ftpBrowserErrorContainer").style.display="none";if(!empty(a.breadcrumbs)){e.style.display="block";e.innerHTML="";var c="/";akeeba.Configuration.FtpBrowser.addCrumb(Joomla.JText._("COM_AKEEBA_FILEFILTERS_LABEL_UIROOT"),"/",e,akeeba.Configuration.SftpBrowser);
for(i=0;i<a.breadcrumbs.length;i++){var f=a.breadcrumbs[i],c=c+("/"+f);akeeba.Configuration.FtpBrowser.addCrumb(f,c,e,akeeba.Configuration.SftpBrowser)}}if(!empty(a.list)){d.style.display="block";akeeba.Configuration.SftpBrowser.params.directory||(akeeba.Configuration.SftpBrowser.params.directory=a.directory);for(i=0;i<a.list.length;i++)e=a.list[i],akeeba.Configuration.FtpBrowser.createLink(a.directory+"/"+e,e,d,akeeba.Configuration.SftpBrowser)}}},function(a){document.getElementById("sftpBrowserError").innerHTML=
a;document.getElementById("sftpBrowserErrorContainer").style.display="block";document.getElementById("sftpBrowserFolderList").style.display="none";document.getElementById("sftpBrowserCrumbs").style.display="none"},!1)};akeeba.Configuration.SftpBrowser.callback=function(a){var b="/".replace(/([\[\]\(\)\.\?\/\*\{\}\+\$\^\:])/g,"$1");a="/"+(a+"").replace(RegExp("^["+b+"]+","g"),"");document.getElementById("var["+akeeba.Configuration.SftpBrowser.params.key+"]").value=a};
...
d=document.getElementById("testFtpDialogBodyFail"),e=document.getElementById("testFtpDialogLabel");b.style.display="none";d.style.display="none";!0===a?(e.textContent=Joomla.JText._("COM_AKEEBA_CONFIG_DIRECTFTP_TEST_OK"),b.textContent=Joomla.JText._("COM_AKEEBA_CONFIG_DIRECTFTP_TEST_OK"),b.style.display="block",d.style.display="none"):(e.textContent=Joomla.JText._("COM_AKEEBA_CONFIG_DIRECTFTP_TEST_FAIL"),d.textContent=a,b.style.display="none",d.style.display="block");akeeba.Modal.open({inherit:"#testFtpDialog",
...
b+".privkey]").value,pubkey:document.getElementById("var["+b+".pubkey]").value};akeeba.System.params.AjaxURL=akeeba.Configuration.URLs.testSftp;akeeba.System.doAjax(a,function(a){document.getElementById(akeeba.Configuration.SftpTest.buttonKey).removeAttribute("disabled");var b=document.getElementById("testFtpDialogBodyOk"),d=document.getElementById("testFtpDialogBodyFail"),e=document.getElementById("testFtpDialogLabel");b.style.display="none";d.style.display="none";!0===a?(e.textContent=Joomla.JText._("COM_AKEEBA_CONFIG_DIRECTSFTP_TEST_OK"),
b.textContent=Joomla.JText._("COM_AKEEBA_CONFIG_DIRECTSFTP_TEST_OK"),b.style.display="block",d.style.display="none"):(e.textContent=Joomla.JText._("COM_AKEEBA_CONFIG_DIRECTSFTP_TEST_FAIL"),d.textContent=a,b.style.display="none",d.style.display="block");akeeba.Modal.open({inherit:"#testFtpDialog",width:"80%"})},null,!1,15E3)};
...
b["default"]==key&&l.setAttribute("selected","selected");k.appendChild(l)}akeeba.System.addEventListener(k,"change",function(a){a={};var b=[document.getElementById(g+"_config").querySelectorAll("input"),document.getElementById(g+"_config").querySelectorAll("select")],c=null,d=null,e=null;for(i=0;i<b.length;i++)if(c=b[i],c.length)for(j=0;j<c.length;j++)d=c[j],e=d.id,a[e]=d.value,"checkbox"==d.getAttribute("type")?a[e]=d.checked:"select"==d.getAttribute("type")&&(a[e]=d.options[d.selectedIndex].value);
var l=akeeba.Configuration.engines[f][k.value],b={};b[l.information.title]=l.parameters;akeeba.Configuration.parseGuiData(b,h);b=h.querySelector("header");b instanceof Element&&b.insertAdjacentHTML("afterend",'<p class="akeeba-block--information">'+l.information.description+"</p>");b=[document.getElementById(g+"_config").querySelectorAll("input"),document.getElementById(g+"_config").querySelectorAll("select")];for(i=0;i<b.length;i++)if(c=b[i],c.length)for(j=0;j<c.length;j++)d=c[j],e=d.id,e=a[e],null==
e||void 0==e||("checkbox"==d.getAttribute("type")?e?d.setAttribute("checked","checked"):d.removeAttribute("checked"):d.value=e,1==i&&akeeba.System.triggerEvent(d,"change"));if("undefined"!==typeof l.information.activation_callback)window[l.information.activation_callback](l.parameters)});a=document.createElement("button");a.className="akeeba-btn--small--dark";c=document.createElement("span");c.className="akion-wrench";a.appendChild(c);c=document.createElement("span");c.innerHTML=Joomla.JText._("COM_AKEEBA_CONFIG_UI_CONFIG");
...
akeeba.Configuration.GUI.renderOptionTypeBrowsedir=function(a,b,d,e,c){var f=document.createElement("input");f.setAttribute("type","text");f.setAttribute("name",a);f.setAttribute("size","30");f.id=a;f.value=b["default"];a=document.createElement("button");a.className="akeeba-btn--grey";a.setAttribute("title",Joomla.JText._("COM_AKEEBA_CONFIG_UI_BROWSE"));b=document.createElement("span");b.className="akion-folder";a.appendChild(b);akeeba.System.addEventListener(a,"click",function(a){a.preventDefault?
...
akeeba.Configuration.GUI.renderOptionTypeButtonedit=function(a,b,d,e,c){var f="hidden"==b.editortype?"hidden":"text";c=document.createElement("input");c.className="akeeba-configuration-buttonedit";c.setAttribute("type",f);c.setAttribute("name",a);c.setAttribute("size",30);c.value=b["default"];"1"==b.editordisabled&&c.setAttribute("disabled","disabled");a=document.createElement("button");a.innerHTML=Joomla.JText._(b.buttontitle);a.className="btn";akeeba.System.addEventListener(a,"click",function(a){a.preventDefault?
a.preventDefault():a.returnValue=!1;a=b.hook;try{eval(a+"()")}catch(c){}});f=document.createElement("span");f.className="input-append";f.appendChild(c);f.appendChild(a);d.appendChild(f);e.appendChild(d)};
...
akeeba.Configuration.GUI.renderOptionTypeButton=function(a,b,d,e,c){c=e.querySelector("label");var f=b.hook;b=c.innerHTML;var g=document.createElement("button");g.id=a;g.innerHTML=b;g.className="btn";c.innerHTML="&nbsp;";akeeba.System.addEventListener(g,"click",function(a){a.preventDefault?a.preventDefault():a.returnValue=!1;try{eval(f+"()")}catch(b){}});d.appendChild(g);e.appendChild(d)};
...
akconfig_onedrivebusiness_oauth_callback=function(a){document.getElementById("var[engine.postproc.onedrivebusiness.access_token]").value=a.access_token;document.getElementById("var[engine.postproc.onedrivebusiness.refresh_token]").value=a.refresh_token;myWindow=window.open("","akeeba_onedrivebusiness_window");myWindow.close()};
...
"engine.postproc.googledrive.refresh_token":document.getElementById("var[engine.postproc.googledrive.refresh_token]").value}};akeeba.System.doAjax(b,function(b){if(0===b.length)alert("ERROR: Could not retrieve list of Google Drives.");else{var e=document.getElementById("var[engine.postproc.googledrive.team_drive]");e.innerHTML="";for(var c=0;c<b.length;c++){var f=document.createElement("option");f.value=b[c][0];f.text=b[c][1];a["engine.postproc.googledrive.team_drive"]["default"]===f.value&&(f.selected=
!0);e.appendChild(f)}}},function(a){alert("ERROR: Could not retrieve list of Google Drives. Error: \n"+a)},!1,15E3)};akconfig_box_openoauth=function(){var a=akeeba.Configuration.URLs.dpeauthopen,a=-1==a.indexOf("?")?a+"?":a+"&";window.open(a+"engine=box","akeeba_box_window","width=1010,height=500")};
akconfig_box_oauth_callback=function(a){document.getElementById("var[engine.postproc.box.access_token]").value=a.access_token;document.getElementById("var[engine.postproc.box.refresh_token]").value=a.refresh_token;myWindow=window.open("","akeeba_box_window");myWindow.close()}; //# sourceMappingURL=Configuration.min.map
